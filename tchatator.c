...
int main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        else if (strncmp(trimmed_buffer, "/liste", 6) == 0)
        {
            ...
            {
                char *page = strtok(trimmed_buffer + 6, " ");
                if (page == NULL)
                {
                    page = "0";
                }
                char *param = get_param(params, "max_messages");
                char query[256];
                snprintf(query, sizeof(query), "SELECT * FROM sae_db._message WHERE (id_receveur = '%s' OR id_envoyeur = '%s') ORDER BY date_envoi DESC LIMIT %d OFFSET %d;", id_compte_client, id_compte_client, atoi(param), atoi(page) * atoi(param));
                PGresult *res = execute(conn, query);

                int nb_messages = PQntuples(res);
                for (int i = 0; i < nb_messages; i++)
                {
                    char *message = PQgetvalue(res, i, 3);
